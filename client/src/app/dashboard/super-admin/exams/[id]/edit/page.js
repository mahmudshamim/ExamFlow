    ...
    useEffect(() => {
        const fetchExamData = async () => {
            try {
                ...
                const { exam, questions: fetchedQuestions } = res.data;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const swalResult = await Swal.fire({
                html: `
                    <div class="flex flex-col items-center">
                        ...
                        <h2 class="text-3xl font-black text-slate-900 mb-2 leading-tight text-center">Changes Saved!</h2>
                        <p class="text-slate-500 mb-6 text-center text-sm px-4">Your assessment has been updated successfully.</p>
            ...
            if (swalResult.isConfirmed) {
                router.push('/dashboard/super-admin/results');
            }
        } catch (err) {
        ...
        <div className="max-w-4xl mx-auto pb-20">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between sticky top-0 md:static z-20 bg-gray-50/90 backdrop-blur-md py-4 mb-6 gap-4">
                <div className="flex items-center gap-4 w-full md:w-auto">
                    <Link href="/dashboard/super-admin/results" className="p-2 hover:bg-gray-200 rounded-lg transition-colors text-gray-600">
                        <ChevronLeft size={24} />
                    </Link>
                    <div>
                        <h1 className="text-xl md:text-2xl font-bold text-gray-900">Edit Assessment</h1>
                        <p className="text-xs md:text-sm text-gray-500 hidden md:block">Update exam questions and settings</p>
                    </div>
                </div>
                                ...
                                <div className="flex items-center gap-3 bg-red-50/50 px-4 py-2 rounded-xl border border-red-100 transition-all hover:bg-red-50/80 animate-in zoom-in duration-300">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <input
                                                type="number"
                                                value={examData.settings.tabSwitchLimit || 0}
                                                onChange={e => {
                                                    const val = parseInt(e.target.value);
                                                    setExamData({
                                                        ...examData,
